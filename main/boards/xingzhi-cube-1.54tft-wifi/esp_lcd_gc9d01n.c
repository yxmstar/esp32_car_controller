...
static const gc9d01n_lcd_init_cmd_t vendor_specific_init_default[] = {
    ...
    {0x80, (uint8_t[]){0x13}, 1, 0},
    {0x81, (uint8_t[]){0x40}, 1, 0},
    {0x82, (uint8_t[]){0x0A}, 1, 0},
    {0x83, (uint8_t[]){0x0B}, 1, 0},
    {0x84, (uint8_t[]){0x60}, 1, 0},
    {0x85, (uint8_t[]){0x80}, 1, 0},

    {0x89, (uint8_t[]){0x10}, 1, 0},
    {0x8A, (uint8_t[]){0x0F}, 1, 0},
    {0x8B, (uint8_t[]){0x02}, 1, 0},
    {0x8C, (uint8_t[]){0x59}, 1, 0},
    {0x8D, (uint8_t[]){0x55}, 1, 0},

    {0x3A, (uint8_t[]){0x05}, 1, 0},
    {0xEC, (uint8_t[]){0x00}, 1, 0},

    {0x7E, (uint8_t[]){0x10}, 1, 0},

    {0x74, (uint8_t[]){0x06, 0x4E, 0x00, 0x00, 0x00, 0x00, 0x00}, 7, 0},

    {0xB5, (uint8_t[]){0x0D, 0x0D}, 2, 0},
    {0xB6, (uint8_t[]){0x00, 0x60}, 2, 0},

    {0x60, (uint8_t[]){0x38, 0x09, 0x1E, 0x7A}, 4, 0},
    {0x63, (uint8_t[]){0x38, 0x86, 0x1E, 0x7A}, 4, 0},
    {0x64, (uint8_t[]){0x38, 0x0B, 0x70, 0x83, 0x1E, 0x7A}, 6, 0},
    {0x66, (uint8_t[]){0x38, 0x0F, 0x70, 0x87, 0x1E, 0x7A}, 6, 0},

    {0x68, (uint8_t[]){0x00, 0x08, 0x07, 0x00, 0x07, 0x55, 0x6A}, 7, 0},
    {0x6A, (uint8_t[]){0x00, 0x00}, 2, 0},
    {0x6C, (uint8_t[]){0x22, 0x02, 0x22, 0x02, 0x22, 0x22, 0x50}, 7, 0},

    {0x6E, (uint8_t[]){
        0x00, 0x00, 0x00, 0x02, 0x14, 0x12, 0x0C, 0x0A, 0x1E, 0x1D,
        0x08, 0x00, 0x16, 0x15, 0x00, 0x00, 0x00, 0x00, 0x15, 0x16,
        0x00, 0x07, 0x1D, 0x1E, 0x09, 0x0B, 0x11, 0x13, 0x01, 0x00,
        0x00, 0x00
    }, 32, 0},
    ...
    {0x70, (uint8_t[]){0x02, 0x01, 0x0D, 0x02, 0x01, 0x0D}, 6, 0},
    {0x71, (uint8_t[]){0x02, 0x01, 0x0D}, 3, 0},

    {0xC3, (uint8_t[]){0x2C}, 1, 0},
    {0xC4, (uint8_t[]){0x1E}, 1, 0},
    {0xC9, (uint8_t[]){0x32}, 1, 0},

    {0xF0, (uint8_t[]){0x48, 0x08, 0x0F, 0x00, 0x00, 0x26}, 6, 0},
    {0xF2, (uint8_t[]){0x47, 0x06, 0x0E, 0x00, 0x00, 0x22}, 6, 0},
    {0xF1, (uint8_t[]){0x42, 0x74, 0x1B, 0x2E, 0x2F, 0xEF}, 6, 0},
    {0xF3, (uint8_t[]){0x3D, 0x74, 0x12, 0x2E, 0x2F, 0xEF}, 6, 0},

    {0xBF, (uint8_t[]){0x00}, 1, 0},
    {0xF9, (uint8_t[]){0x40}, 1, 0},
    {0x36, (uint8_t[]){0x00}, 1, 0},

    {0x2A, (uint8_t[]){0x00, 0x0F, 0x00, 0x40}, 4, 0},
    {0x2B, (uint8_t[]){0x00, 0x00, 0x00, 0x77}, 4, 0},

    {0x11, (uint8_t[]){0x00}, 0, 200},
    {0x29, (uint8_t[]){0x00}, 0, 0},
    {0x2C, (uint8_t[]){0x00}, 0, 20},  // 一般是开始写入数据
};
